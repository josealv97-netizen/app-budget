    ...
    return (
        <div className="space-y-6 h-full flex flex-col">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 h-[45vh] min-h-[400px]">
                ...
                <Card className="flex flex-col relative overflow-hidden bg-white border-slate-200">
                    <div className="flex justify-between items-start mb-4 z-10">
                        <div>
                            <h3 className="text-sm font-semibold text-slate-500 uppercase tracking-wider mb-1">Solde Actuel</h3>
                            <div className="text-3xl font-bold text-slate-900">€{balance.toLocaleString()}</div>
                        </div>
                        <div className="text-right">
                            <h3 className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1">Projection (Fin de mois)</h3>
                            <div className="text-xl font-bold text-blue-600">€{projected.toLocaleString()}</div>
                        </div>
                    </div>
                    ...
                    <div className="flex flex-wrap items-center gap-4 mb-2 z-10">
                        ...
                        <div className="flex gap-1 bg-slate-100 p-1 rounded-lg">
                            {ranges.map(r => (
                                <button
                                    ...
                                    className={`px-3 py-1 rounded-md text-xs font-medium transition-all ${range === r.key ? 'bg-white shadow text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
                                >
                                    {r.label}
                                </button>
                            ))}
                        ...
                        <div className="flex gap-1 bg-slate-100 p-1 rounded-lg">
                            <button
                                ...
                                className={`px-3 py-1 rounded-md text-xs font-medium transition-all ${direction === 'past' ? 'bg-white shadow text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                Historique
                            </button>
                            <button
                                onClick={() => setDirection('future')}
                                className={`px-3 py-1 rounded-md text-xs font-medium transition-all ${direction === 'future' ? 'bg-white shadow text-slate-900' : 'text-slate-500 hover:text-slate-700'}`}
                            >
                                Futur
                            </button>
                        </div>
                                    ...
                                    return (
                                        <tr key={`${t.id}-${i}`} className={`hover:bg-slate-50/50 transition-colors ${t.isFuture ? 'bg-amber-50/30' : ''}`}>
                                            <td className="px-4 py-3 font-medium text-slate-900 flex items-center gap-3">
                                                ...
                                                <div>
                                                    <div className="text-slate-900 font-semibold">{t.description}</div>
                                            ...
                                            <td className={`px-4 py-3 font-bold whitespace-nowrap ${t.type === 'income' ? 'text-green-600' : 'text-slate-900'}`}>
